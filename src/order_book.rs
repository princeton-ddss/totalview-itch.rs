...
impl OrderBook {
    ...
    pub fn sorted_bids(&self) -> Vec<(u32, u32)> {
        let mut bids: Vec<(u32, u32)> = self.bids.iter().map(|(&price, &shares)| (price, shares)).collect();
    ...
    pub fn sorted_asks(&self) -> Vec<(u32, u32)> {
        let mut asks: Vec<(u32, u32)> = self.asks.iter().map(|(&price, &shares)| (price, shares)).collect();
